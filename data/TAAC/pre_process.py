...
def _extract_flat_features(key, feat):
    ...
    if value_type == "int_array_and_float_array":
        # Keep the original int-side column name for compatibility with existing configs.
        # Preserve the float side in an extra column.
        return [
            (f"{key}{feature_id}", _to_seq_string(feat.get("int_array")), "seq_str"),
